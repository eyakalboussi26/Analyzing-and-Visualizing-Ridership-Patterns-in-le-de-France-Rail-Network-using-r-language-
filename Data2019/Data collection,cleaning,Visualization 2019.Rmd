...
```{r}
# Trier les données par 'pourc_validations' de manière décroissante et extraire la première ligne
max_pourc_validations_row <- ProfilFerS1_cleaned %>%
  arrange(desc(pourc_validations)) %>%
  slice(1) %>%
  select(TRNC_HORR_60, LIBELLE_ARRET, CAT_JOUR, pourc_validations)

# Afficher la ligne avec le maximum des pourcentages de validations
...
# Save the graph as an image file (e.g., ".png", ".jpg")
ggsave("Tick_typ_valids_dist_S1_2019.png", Tick_typ_valids_dist_S1, width = 10, height = 6, units = "in")
...
# Save the graph as an image file (e.g., ".png", ".jpg")
ggsave("Tick_typ_valids_dist_S2_2019.png", Tick_typ_valids_dist_S2, width = 10, height = 6, units = "in")

# Display the graph
print(Tick_typ_valids_dist_S2)
```

#### 3. Spatiotemporal Analysis of Transport Ticket Validations during the Second Semester of 2019
...
#### 5. Comparison of Ticket Validation Counts between Summer Vacation and Normal School Periods in 2019
...
# Trouver l'index de la ligne avec la valeur maximale dans la colonne 'pourc_validations'
max_index <- which.max(ProfilFerS1_cleaned$pourc_validations)

# Extraire une sous-section de ProfilFerS1_cleaned pour la ligne avec la valeur maximale
result <- ProfilFerS1_cleaned[max_index, c("TRNC_HORR_60", "LIBELLE_ARRET", "CAT_JOUR", "pourc_validations")]
...
# Remove rows with NA values in the 'NB_VALD' column
ProfilFerS2_cleaned <- ProfilFerS2_cleaned[!is.na(ProfilFerS2_cleaned$NB_VALD), ]
...
Initially, the dataset **"ProfilFerS2"**dimension is 82588. We detected that there are 138 rows containing missing values. We removed them and rechecked the data dimension, which is now 82450.
