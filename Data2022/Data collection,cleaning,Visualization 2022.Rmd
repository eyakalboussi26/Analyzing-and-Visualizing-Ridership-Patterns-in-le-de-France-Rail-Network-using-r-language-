---
title: 'Data collection,cleaning,Visualization 2022'
...
```{r}
##  Load the 2022 data
NbValidS1 <- read.delim("/cloud/project/Data2022/NbValidS1.txt")
NbValidS2 <- read.delim("/cloud/project/Data2022/NbValidS2.txt")
ProfilFerS1 <- read.delim("/cloud/project/Data2022/ProfilFerS1.txt", header=FALSE)
ProfilFerS2 <- read.delim2("/cloud/project/Data2022/ProfilFerS2.txt", header=FALSE)
```
...
Initially, the dataset **"NbValidS1"**dimension is 1088334. We detected that there are 120417rows containing missing values. We removed them and rechecked the data dimension, which is now 967917.
...
plot_result <- ggplot(result, aes(x = JOUR, y = NB_VALD)) +
  ...
  labs(x = "Day", y = "Number of Validations", title = "Day with Maximum number of validations S1 2022")

# Show the graph
print(plot_result)

# Save the graph as an image file (e.g., ".png", ".jpg")
ggsave("Day with Maximum number of validations S1 2022.png", plot_result, width = 10, height = 6, units = "in")
...
Ticket_valD_dist_S1_2022 <- ggplot(NbValidS1_cleaned, aes(x = CATEGORIE_TITRE, y = NB_VALD, fill = CATEGORIE_TITRE)) +
  geom_bar(stat = "identity") +
  labs(x = "Category of Ticket", y = "Number of Validations", title = "Distribution of validations for Ticket Transport category S2 2022") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

# Save the graph with the file extension (e.g., ".png", ".jpg")
ggsave("Ticket_valD_dist_S1_2022.png", Ticket_valD_dist_S1_2022, width = 10, height = 6, units = "in")

# Displaying the graph
print(Ticket_valD_dist_S1_2022)
...
-   During the first semester of 2022, starting in January and ending in June, our graphical analyses revealed that "Navigo" had the highest number of validations among all transportation passes 87304.
...
-   "14/06/2022" is the day that had the highest number of validations, totaling 87304
...
Initially, the dataset **"NbValidS2"**dimension is 1105947 . We detected that there are 251185 rows containing missing values. We removed them and rechecked the data dimension, which is now 974837
...
library(ggplot2)
# Creating a graph representing the distribution of ticket categories by number of validations for these two lines
Ticket_valD_dist_S2_2022 <- ggplot(NbValidS2_cleaned, aes(x = CATEGORIE_TITRE, y = NB_VALD, fill = CATEGORIE_TITRE)) +
  geom_bar(stat = "identity") +
  labs(x = "Category of Ticket", y = "Number of Validations", title = "Distribution of validations for Ticket Transport category S2 2022") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
# Save the graph with the file extension (e.g., ".png", ".jpg")
ggsave("Ticket_valD_dist_S2_2022.png", Ticket_valD_dist_S2_2022, width = 10, height = 6, units = "in")

# Displaying the graph
print(Ticket_valD_dist_S2_2022)
```

#### 3. Spatiotemporal Analysis of Transport Ticket Validations during the Second Semester of 2022

-   During the second semester of 2022, starting in july and ending in decembre, our graphical analyses revealed that "Navigo" had the highest number of validations among all transportation passes 92139.

-   We can conclude that during this period, "Navigo" was the best-selling ticket. Additionally, the stations recording the highest number of validations is Saint-Lazare station.

-   The Saint-Lazare station is served by regional trains, long-distance trains, and suburban lines, facilitating travel for both local and national passengers

-   Following Navigo, the transportation passes "Imagine r" and "TST"were also notable.Also We found that the stations "BOULOGNE-ST.CL"" and \
    \* CHARDON-LAGACH "" have the fewest validations, specifically fewer than 5 With transport ticket title is "Navigo Jour" at 01/07/2022.
...
# Specific dates
dates <- c("2022-07-01", "2022-07-14", "2022-12-31", "2022-12-25")
...
We found that on the date 2022-12-25, there were the fewest validations, equal to , which confirms our hypothesis as the French people stay at home to celebrate Christmas with family and friends.

The date 2022-07-01 had the highest number of validations: 4714903. Additionally, we observed a significant number of validations on 14/07/2022, indicating higher public movement due to the celebration of the French National Day."
...
ggplot(results, aes(x = Date, y = Validations)) +
  ...
  labs(title = "Validation Counts for Specific Dates in 2022",
       x = "Date",
       y = "Validation Counts") +
...
#### 5. Comparison of Ticket Validation Counts between Summer Vacation and Normal School Periods in 2022
...
summer_vacation_start <- as.Date("2022-07-08")
summer_vacation_end <- as.Date("2022-09-03")
normal_period_start <- as.Date("2022-09-04")
normal_period_end <- as.Date("2022-10-10")
...
-   **Effect of Summer Vacation on IMAGINE R Transport Ticket Validations in Two Semesters of 2022**

To assess the impact of summer vacation on the validation count of IMAGINE R transport tickets, we compared two periods within the second semester of 2022.
...
Initially, the dataset **"ProfilFerS1"**dimension is 832. We detected that there are 277 rows containing missing values. We removed them and rechecked the data dimension, which is now 82984.
...
# Remove rows with specific or missing values
ProfilFerS2_cleaned <- ProfilFerS[-rows_to_remove, ]
...
Initially, the dataset **"ProfilFerS2"**dimension is 85255. We detected that there are 118 rows containing missing values. We removed them and rechecked the data dimension, which is now 85137.
